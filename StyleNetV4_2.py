...
def enc_unit(inp, filter_mult=1, name='enc', first=False):
    ...
    x1 = conv2D(num_filters * filter_mult, kernel_size=(3, 3), strides=(2, 2), padding='same')(inp)
    ...
    x2 = conv2D(num_filters * filter_mult * 2, kernel_size=(3, 3), strides=(2, 2), padding='same')(x1)
...
class AE_A(tf.keras.Model):
    ...
    def train_model(self, fname=None):
        ...
        for i in range(num_epochs):
            ...
            start = time.time()
            batch_on = 0
            for aaa, aaa in zip(image_dataset.take(int(dataset_size / 20)), image_dataset.take(int(dataset_size / 20))):
                #try:
                loss_identity, kl_loss, style_loss = self.train_step(source, style, optimizer)
                if batch_on % record_batch == 0:
                    print("Beginning batch #" + str(batch_on), 'out of', int(dataset_size / 20), 'of size', batch_size)
            ...
            if i % record_after_epochs == 0:
                ...
                self.loss['style'] = self.style_loss
                #for style in zip(image_dataset.take(1)):
                print(style.shape, source.shape)
    ...
    def train_step(self, source, style, optimizer):
        with tf.GradientTape() as tape:
            ...
            style_loss = tf.add_n([tf.reduce_mean((utils.gram_matrix(st) - utils.gram_matrix(self.encode(y))) ** 2)])

            # reduce_mean Computes the mean of elements across dimensions of a tensor.
            # add_n Adds all input tensors element-wise.
            style_loss *= style_weight
            prediction = self(source)
            loss_identity = identity_lr * learning_rate * (0.2 * tf.reduce_mean(tf.reduce_sum(cross_entropy(source, prediction))) + tf.reduce_mean(tf.reduce_sum((source-prediction)**2)))
