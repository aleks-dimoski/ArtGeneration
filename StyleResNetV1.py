...
def dec_unit(inp, filter_mult=1, name='dec', last=False, first=False):
    ...
    if not last:
        ...
        x = conv2DT(64, (1, 1), strides=(1, 1), padding='same')(inp)
        x = tf.keras.layers.LeakyReLU()(x)
...
class AE_A(tf.keras.Model):
    def __init__(self, training=True):
        ...
        self.res50 = tf.keras.applications.ResNet50(include_top=False, weights='imagenet', input_tensor=inp1)
        self.res50.trainable = False
        ...
        self.skip4 = latent_skip(inp5, latent_dims=128*num_filters, name='latent_skip4', training=training) #256
        self.skip3 = latent_skip(inp4, latent_dims=64*num_filters, name='latent_skip3', training=training) #128
        self.skip2 = latent_skip(inp3, latent_dims=16*num_filters, name='latent_skip2', training=training) #32
        self.skip1 = latent_skip(inp2, latent_dims=8*num_filters, name='latent_skip1', training=training) #16'''

        out5 = tf.keras.Input(shape=(self.res50.output_shape[1:]))
        ...
        self.dec1 = dec_unit(inp=out1, last=True, filter_mult=2, name='D1')
        ...
        for i in range(num_epochs):
            ...
            if i % display_mod == 0:
                ...
                for style in zip(image_dataset.take(1)):
                    utils.test_model(model_val, source, style=None, num=i, test=True, name=model_name, details='merge')
                    break
